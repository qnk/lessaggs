...
function gk_post_meta($attachment = false) {
 	...
 	?> 
	 	...
	 	<ul>
	 		...
	 		<?php if(((is_tag() || is_search()) && $param_category) ) : ?>
	 		<li class="category">
	 			<?php _e('Published in', GKTPLNAME); ?>
	 			<?php echo get_the_category_list( __(', ', GKTPLNAME )); ?>
	 		</li>
...
function gk_social_api($title, $postID) {
	...
	if($social_api_mode != 'none' && $is_excluded) {
		...
		if(get_option($gk_tpl->name . '_fb_like', 'Y') == 'Y') {
			...
			$fb_like_output = '<div class="item-fblike-button"><div class="fb-like" data-href="'.get_permalink($postID).'" '.$fb_like_attributes.'></div></div>';
		}
		// G+
		if(get_option($gk_tpl->name . '_google_plus', 'Y') == 'Y') {
			...
			$gplus_output = '<div class="item-gplus-button"><g:plusone '.$gplus_attributes.' callback="'.get_permalink($postID).'"></g:plusone></div>';
		}
		// Twitter
		if(get_option($gk_tpl->name . '_tweet_btn', 'Y') == 'Y') {
			...
			$twitter_output = '<div class="item-twitter-button"><a href="http://twitter.com/share" class="twitter-share-button" data-text="'.strip_tags($title).'" data-url="'.get_permalink($postID).'" '.$tweet_btn_attributes.'>'.__('Tweet', GKTPLNAME).'</a></div>';
		}
		// Pinterest
