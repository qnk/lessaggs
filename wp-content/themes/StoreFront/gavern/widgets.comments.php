			...
			if(count($comments) > 0) {				
				...
				for($i = 0; $i < count($comments); $i++) {
					echo '<li'.(user_can($comments[$i]->user_id, 'delete_users') ? ' class="gk-admin-comment"' : '').'>';
						echo '<div><p><a href="'.get_comment_link($comments[$i]->comment_ID).'">' . $this->comment_text($comments[$i]->comment_content, $word_count) . '</a></p></div>';
						
						echo '<p>' .get_avatar($comments[$i]->comment_author_email, $avatar_size);
						echo '<strong>' . $comments[$i]->comment_author . '</strong>';
						echo '<small>' . $comments[$i]->comment_date_gmt . '</small></p>';				
					echo '</li>';
				}
